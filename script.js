document.addEventListener('DOMContentLoaded', function() {
    ...
    const printIndividuallyBtn = document.getElementById('printIndividuallyBtn');
    ...
    const requiredFields = ['recallDate', 'manufacturer', 'ncr', 'materialCategory', 'issue'];
    ...
    function validateField(field) {
        ...
        switch (fieldName) {
            case 'recallDate':
                if (!value) {
                    errorMessage = 'Recall date is required';
                } else {
                    const date = new Date(value);
                    const today = new Date();
                    if (date > today) {
                        errorMessage = 'Recall date cannot be in the future';
                    }
                }
                break;
            case 'manufacturer':
                if (!value) {
                    ...
                    errorMessage = 'Manufacturer name must be at least 2 characters';
                }
                break;
            case 'ncr':
                if (!value) {
                    errorMessage = 'NCR is required';
                }
                break;
    ...
    function safeReplacePlaceholders(text, replacements) {
        if (!text) return '';
        ...
        Object.keys(replacements).forEach(placeholder => {
            ...
            if (value !== null && value !== undefined && value.toString().trim() !== '') {
                ...
                result = result.replace(regex, sanitizeHtml(value.toString()));
            } else {
        ...
        // Clean up any remaining empty lines or extra spaces
        result = result.replace(/\n\s*\n\s*\n/g, '\n\n'); // Remove triple line breaks
        result = result.replace(/^\s+|\s+$/g, ''); // Trim whitespace
    ...
    // Next Revisions feedback modal functionality
    const nextRevisionsBtn = document.getElementById('nextRevisionsBtn');
    const nextRevisionsModal = document.getElementById('nextRevisionsModal');
    const closeNextRevisionsBtn = document.getElementById('closeNextRevisionsModal');
    if (nextRevisionsBtn && nextRevisionsModal && closeNextRevisionsBtn) {
        nextRevisionsBtn.addEventListener('click', () => {
            nextRevisionsModal.classList.remove('hidden');
        });
        closeNextRevisionsBtn.addEventListener('click', () => {
            nextRevisionsModal.classList.add('hidden');
        });
        window.addEventListener('click', (e) => {
            if (e.target === nextRevisionsModal) {
                nextRevisionsModal.classList.add('hidden');
            }
        });
    }
    ...
    if (printIndividuallyBtn) printIndividuallyBtn.addEventListener('click', printLettersIndividually);
    ...
    const updateLettersUI = function() {
        ...
        if (buttonsContainer) {
            // Add Print Letters Individually button if not already present
            if (!document.getElementById('printIndividuallyBtn')) {
                const printIndividuallyBtn = document.createElement('button');
                printIndividuallyBtn.id = 'printIndividuallyBtn';
                printIndividuallyBtn.className = 'btn';
                printIndividuallyBtn.textContent = 'Print Letters Individually';
                printIndividuallyBtn.addEventListener('click', printLettersIndividually);
                buttonsContainer.appendChild(printIndividuallyBtn);
            }
    ...
    // Handle CSV upload with improved error handling
    function handleCSVUpload(event) {
        const file = event.target.files[0];
        if (!file) return;
        
        // Clear any existing CSV errors
        const existingErrors = document.querySelectorAll('.csv-error-container');
        ...
        Papa.parse(file, {
            ...
            complete: function(results) {
                ...
                if (csvData.length === 0) {
                    showCSVError('No data found in the CSV file. Please check that the file contains data rows.');
                    return;
                }
            ...
            error: function(error) {
                ...
                showCSVError(`Error parsing the CSV file: ${error.message || 'Unknown error'}. Please check the file format.`);
            }
        });
    }
    
    // Show CSV parsing errors with details
    function showCSVErrors(errors) {
        const errorContainer = document.createElement('div');
        errorContainer.className = 'csv-error-container';
        
        const title = document.createElement('div');
        title.className = 'csv-error-title';
        title.textContent = 'CSV Parsing Errors:';
        errorContainer.appendChild(title);
        
        errors.forEach((error, index) => {
            const detail = document.createElement('div');
            detail.className = 'csv-error-details';
            detail.textContent = `Error ${index + 1}: ${error.message}`;
            if (error.row !== undefined) {
                detail.textContent += ` (Row: ${error.row + 1})`;
            }
            errorContainer.appendChild(detail);
        });
        
        csvUpload.parentNode.appendChild(errorContainer);
    }
    
    // Show single CSV error
    ...
    function createFormattingToolbar(targetId) {
        ...
        const colors = [
            ...
            {name: 'Company Blue', value: '#00447c'}
        ];
    ...
    // Generate letters with loading indicator and improved validation
    function generateLetters() {
        ...
        if (csvData.length === 0) {
            showCSVError('Please upload a CSV file with customer data.');
            return;
        }
        ...
        loadingDiv.innerHTML = '<div class="loading-spinner"></div><p>Generating letters...</p>';
        ...
        setTimeout(() => {
            try {
                ...
                alert('An error occurred while generating letters. Please check your data and try again.');
            } finally {
    ...
    // Group data by specified criteria with improved SSR logic
    function groupDataBy(data, groupBy) {
        ...
        switch (groupBy) {
            ...
            case 'SSR':
                data.forEach(row => {
                    const ssr = row['Sales Representative'] || 'Unknown';
                    const address = row.Address || 'Unknown';
                    // Use combination of SSR and address to avoid duplicates for same rep
                    const key = `${ssr}-${address}`;
                    if (!groupedData[key]) {
                        groupedData[key] = [];
                    }
                    groupedData[key].push(row);
    ...
    // Create a letter element with improved placeholder handling
    function createLetter(records, recallDate, manufacturer, ncr, materialCategory, issue, selectedFields, heatNumber, 
        ...
        // Add address under the logo with proper line breaks
        ...
        if (isContentOverride && overrideText) {
            ...
            const formattedOverrideText = safeReplacePlaceholders(overrideText, replacements);
            ...
            // Use standard paragraphs with safe formatting
            const formattedParagraph1 = safeReplacePlaceholders(paragraph1, replacements);
            const formattedParagraph2 = safeReplacePlaceholders(paragraph2, replacements);
            const formattedParagraph3 = safeReplacePlaceholders(paragraph3, replacements);
            const formattedParagraph4 = safeReplacePlaceholders(paragraph4, replacements);
            const formattedParagraph5 = safeReplacePlaceholders(paragraph5, replacements);
            const formattedContactInfo = safeReplacePlaceholders(contactInfo, replacements);
    ...
    function updateLetterBeforeExport(letterElement, letterData) {
        ...
        if (letterData.isContentOverride && letterData.overrideText) {
            ...
            if (contentDiv) {
                ...
                const formattedOverrideText = safeReplacePlaceholders(letterData.overrideText, letterData.customFields);
    ...
    // Convert letter HTML to text format with improved reliability
    function convertLetterToText(letterData) {
        ...
        // Clean address (remove HTML and preserve line breaks)
        const cleanAddress = stripHtml(letterData.address).replace(/\n+/g, '\n');
        text += `${cleanAddress}\n\n`;
        ...
        // Add heat number if provided
        let heatNumberText = '';
        if (letterData.heatNumber && letterData.heatNumber.trim()) {
            heatNumberText = ` | Heat Number: ${letterData.heatNumber}`;
        }
        ...
        // Prepare replacement map for placeholders
        const replacements = {
            'MANUFACTURER': letterData.manufacturer,
            'MATERIAL_CATEGORY': letterData.materialCategory,
            'ISSUE': letterData.issue,
            'HEAT_NUMBER': letterData.heatNumber,
            'FIELD1': letterData.customFields.field1,
            'FIELD2': letterData.customFields.field2,
            'FIELD3': letterData.customFields.field3,
            'FIELD4': letterData.customFields.field4,
            'FIELD5': letterData.customFields.field5
        };
        
        // Add body content - check if using override content
        if (letterData.isContentOverride && letterData.overrideText) {
            // Format and add override text
            const formattedOverrideText = safeReplacePlaceholders(letterData.overrideText, replacements);
            const cleanOverrideText = stripHtml(formattedOverrideText);
            text += `${cleanOverrideText}\n\n`;
        } else {
            // Add standard body paragraphs with safe replacement
            const formattedParagraph1 = stripHtml(safeReplacePlaceholders(letterData.paragraph1, replacements));
            const formattedParagraph2 = stripHtml(safeReplacePlaceholders(letterData.paragraph2, replacements));
            const formattedParagraph3 = stripHtml(safeReplacePlaceholders(letterData.paragraph3, replacements));
            const formattedParagraph4 = stripHtml(safeReplacePlaceholders(letterData.paragraph4, replacements));
            const formattedParagraph5 = stripHtml(safeReplacePlaceholders(letterData.paragraph5, replacements));
            const formattedContactInfo = stripHtml(safeReplacePlaceholders(letterData.contactInfo, replacements));
            
            if (formattedParagraph1.trim()) text += `${formattedParagraph1}\n\n`;
            if (formattedParagraph2.trim()) text += `${formattedParagraph2}\n\n`;
            if (formattedParagraph3.trim()) text += `${formattedParagraph3}\n\n`;
            if (formattedParagraph4.trim()) text += `${formattedParagraph4}\n\n`;
            if (formattedParagraph5.trim()) text += `${formattedParagraph5}\n\n`;
            if (formattedContactInfo.trim()) text += `${formattedContactInfo}\n\n`;
        }
        ...
        if (selectedFields.length > 0 && letterData.records && letterData.records.length > 0) {
            ...
            selectedFields.forEach(field => {
                ...
                columnWidths[field] = Math.min(maxWidth + 2, 20); // Cap at 20 characters
            });
            
            // Add header row with proper spacing
            const headerRow = selectedFields.map(field => {
                return field.padEnd(columnWidths[field]);
            }).join('|');
            text += headerRow + '\n';
            
            // Add separator
            const separator = selectedFields.map(field => {
                return '-'.repeat(columnWidths[field]);
            }).join('|');
            text += separator + '\n';
            ...
            letterData.records.forEach(record => {
                const row = selectedFields.map(field => {
                    const value = (record[field] || '').toString();
                    return value.substring(0, columnWidths[field] - 2).padEnd(columnWidths[field]);
                }).join('|');
                text += row + '\n';
            });
        } else {
    ...
    // Print letters individually - creates separate PDF files for each letter
    async function printLettersIndividually() {
        if (generatedLetters.length === 0) {
            alert('No letters to print. Please generate letters first.');
            return;
        }
        ...
        const loadingDiv = document.createElement('div');
        loadingDiv.className = 'loading-overlay';
        loadingDiv.innerHTML = '<div class="loading-spinner"></div><p>Preparing individual letter files for printing...</p>';
        document.body.appendChild(loadingDiv);
        ...
        try {
            ...
            for (let i = 0; i < letterElements.length; i++) {
                ...
                // Create filename for individual printing
                const fileName = `PRINT_${letterData.ncr}_${letterData.control}_${letterData.manufacturer}_${formattedDate}.pdf`;
            ...
            const url = URL.createObjectURL(zipContent);
            a.href = url;
            a.download = `Individual_Print_Files_${new Date().toISOString().slice(0,10)}.zip`;
            document.body.appendChild(a);
            a.click();
            ...
            console.error('Error generating individual print files:', error);
            alert('There was an error generating the individual print files. Please try again.');
        } finally {
            // Remove loading indicator
            document.body.removeChild(loadingDiv);
        }
